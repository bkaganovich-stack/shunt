...
-- No secondary indexes here on purpose. `hour` is the leading column of the
-- primary key, so its autoindex already serves the hour-range scans that both
-- get_traffic_summary() and purge_old_traffic() do. An index on src_ip is worse
-- than useless: behind the router's NAT nearly every row shares one source
-- address. The two indexes they replace cost 31 MB of a 112 MB database.

-- Resource metrics, stored RRD-style: three fixed-resolution tiers, each with a
-- hard row budget, so the table is bounded by construction rather than by hoping
-- a retention job runs. Worst case ~5.6k rows (a few MB), unlike traffic_hourly
-- which reached 600k rows before its indexes were pruned.
--   m1: 60s   step, 24h depth  -> 1440 rows
--   m5: 300s  step, 7d  depth  -> 2016 rows
--   h1: 3600s step, 90d depth  -> 2160 rows
CREATE TABLE IF NOT EXISTS metrics (
    res      TEXT    NOT NULL,   -- m1 | m5 | h1
    ts       INTEGER NOT NULL,   -- unix seconds, floored to the tier step
    cpu      REAL,               -- percent 0..100
    mem      REAL,               -- percent 0..100
    vpn_bps  REAL,               -- bytes/sec through the tunnel
    tot_bps  REAL,               -- bytes/sec total through the gateway LAN side
    PRIMARY KEY (res, ts)
);
...
# ── Resource metrics ──────────────────────────────────────────────────────────
TIERS = {"m1": (60, 1440), "m5": (300, 2016), "h1": (3600, 2160)}


def record_metric(ts: int, cpu: float, mem: float,
                  vpn_bps: float, tot_bps: float) -> None:
    """Store one m1 sample, then roll it up and enforce every tier's row budget."""
    step, _ = TIERS["m1"]
    slot = ts - (ts % step)
    with get_conn() as conn:
        conn.execute(
            "INSERT INTO metrics (res, ts, cpu, mem, vpn_bps, tot_bps) "
            "VALUES ('m1', ?, ?, ?, ?, ?) "
            "ON CONFLICT(res, ts) DO UPDATE SET "
            "cpu=excluded.cpu, mem=excluded.mem, "
            "vpn_bps=excluded.vpn_bps, tot_bps=excluded.tot_bps",
            (slot, cpu, mem, vpn_bps, tot_bps))
        for tier, src in (("m5", "m1"), ("h1", "m5")):
            tstep, _ = TIERS[tier]
            tslot = ts - (ts % tstep)
            conn.execute(
                f"INSERT INTO metrics (res, ts, cpu, mem, vpn_bps, tot_bps) "
                f"SELECT ?, ?, AVG(cpu), AVG(mem), AVG(vpn_bps), AVG(tot_bps) "
                f"FROM metrics WHERE res=? AND ts>=? AND ts<? "
                f"ON CONFLICT(res, ts) DO UPDATE SET "
                f"cpu=excluded.cpu, mem=excluded.mem, "
                f"vpn_bps=excluded.vpn_bps, tot_bps=excluded.tot_bps",
                (tier, tslot, src, tslot, tslot + tstep))
        for tier, (tstep, rows) in TIERS.items():
            conn.execute(
                "DELETE FROM metrics WHERE res=? AND ts < ?",
                (tier, ts - tstep * rows))


def get_metrics(res: str, limit: int = 2000) -> list[dict]:
    """Return samples for one tier, oldest first."""
    if res not in TIERS:
        res = "m1"
    with get_conn() as conn:
        rows = conn.execute(
            "SELECT ts, cpu, mem, vpn_bps, tot_bps FROM metrics "
            "WHERE res=? ORDER BY ts DESC LIMIT ?", (res, limit)).fetchall()
