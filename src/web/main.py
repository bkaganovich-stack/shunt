...
DNS_CONF  = Path("/etc/dnsmasq.d/shunt.conf")
...
def _wire_features() -> None:
    _db.set_db_path(CFG_DIR / "shunt.db")
    _ft.BASE    = BASE
    _ft.CFG_DIR = CFG_DIR
...
def build_dnsmasq_conf(dns: dict) -> str:
    """
    The whole dnsmasq configuration, not only the DNS half.

    apply_topology.py writes this same file with the interface binding and the
    DHCP server the downstream router leases its WAN address from. This function
    used to emit neither, so saving DNS settings replaced that file with one
    that served no DHCP -- and the router silently stopped getting an address at
    its next renewal. Two writers, one file, incompatible content. Whichever
    runs last must produce something complete.
    """
    ...
    try:
        for line in (CFG_DIR / "network.conf").read_text().splitlines():
            if line.startswith("LAN_IF="):
                lan_if = line.split("=", 1)[1].strip()
    except Exception:
        pass
    lines = ["# Generated by shunt — do not edit manually"]
    if lan_if:
        lines.append(f"interface={lan_if}")
    else:
        lines.append(f"listen-address={lan_ip}")
    lines += [
        # bind-dynamic rather than bind-interfaces: the latter claims its
        # sockets once at start-up, so a gateway booted with the LAN cable out
        # never serves anything on that interface afterwards.
        "bind-dynamic",
        "port=5335",
        "no-resolv",
    ...
    # The DHCP half belongs to the topology, but it lives in this file, so it
    # has to be reproduced here or writing DNS settings would delete it.
    if lan_if and lan_ip and lan_ip != "127.0.0.1":
        net = lan_ip.rsplit(".", 1)[0]
        lines += [
            "",
            "# DHCP for the downstream router's WAN port",
            "dhcp-authoritative",
            f"dhcp-range={net}.10,{net}.50,255.255.255.0,12h",
...
def restore_snapshot(snap_id: str) -> tuple[bool, str]:
    ...
    r = subprocess.run(["systemctl", "restart", "shunt"], capture_output=True, text=True)
...
async def _xray_fd_watchdog() -> None:
    """Monitor shunt file descriptor usage and restart before exhaustion.
    ...
    while True:
        try:
            ...
            r = subprocess.run(
                ["systemctl", "show", "-p", "MainPID", "shunt"],
                capture_output=True, text=True)
...
def _restart_xray_watchdog(reason: str) -> None:
    """Restart shunt due to watchdog detection."""
...
async def restart_services(req: SystemActionReq, u: str = Depends(auth_dep)):
    ...
    subprocess.run(
        ["systemd-run", "--unit=gw-restart-services", "--collect", "/bin/sh", "-c",
         "sleep 1; systemctl restart shunt sing-box dnsmasq shunt-dohproxy adguardvpn"],
        capture_output=True)
...
async def test_alert(u: str = Depends(auth_dep)):
    ...
    fire_alert("_test", "Test alert from shunt")
    # Restore
...
async def logs_snapshot(n: int = 300, u: str = Depends(auth_dep)):
    r = subprocess.run(["journalctl", "-u", "shunt", "-n", str(min(n,1000)),
...
async def export_settings(u: str = Depends(auth_dep)):
    ...
    return Response(buf.read(), media_type="application/gzip",
                    headers={"Content-Disposition":f"attachment; filename=shunt-backup-{ts}.tar.gz"})
...
async def import_settings(file: UploadFile = File(...), u: str = Depends(auth_dep)):
    ...
    if any(r.startswith("web/") for r in restored):
        subprocess.Popen(["systemctl","restart","shunt-web"])
