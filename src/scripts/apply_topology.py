...
def write_dnsmasq(t: dict) -> None:
    ...
    if t["topology"] == "inline":
        conf = (f"# Generated by shunt (INLINE) — do not edit manually\n"
                ...
                # bind-dynamic, not bind-interfaces: the latter claims its
                # sockets once at start-up, so a gateway booted with the LAN
                # cable out never serves DHCP or DNS on it afterwards no matter
                # how many times the cable goes back in.
                f"bind-dynamic\n"
                f"port=5335\n"
                f"no-resolv\n"
                f"cache-size=1000\n"
                # The local DNS-over-HTTPS proxy, which is what settings.json
                # already names as the upstream. A hardcoded public resolver
                # here quietly disagreed with the rest of the product.
                f"server=127.0.0.1#5053\n"
