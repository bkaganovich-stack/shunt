#!/usr/bin/env bash
# ─────────────────────────────────────────────────────────────────────────────
# Xray Proxy Gateway — First Boot Setup
# Runs once after Ubuntu autoinstall, via xray-first-boot.service.
# Detects the LAN interface, runs install.sh, then disables itself.
# ─────────────────────────────────────────────────────────────────────────────
set -euo pipefail

LOG=/var/log/xray-first-boot.log
exec > >(tee -a "$LOG") 2>&1

STAMP=/opt/xray-proxy/.installed
SRC=/opt/xray-proxy-src
...
if [[ -f "$STAMP" ]]; then
    ...
    systemctl disable xray-first-boot.service 2>/dev/null || true
    exit 0
fi
...
if [[ ! -d "$SRC" ]]; then
    ...
    echo "        During autoinstall, the CIDATA USB must contain xray-proxy/"
    echo "        which gets copied to /opt/xray-proxy-src by late-commands."
    exit 1
fi
...
# /opt/xray-proxy/config/network.conf for iptables.sh and dnsmasq.
...
# ── Self-disable ──────────────────────────────────────────────────────────────
systemctl disable xray-first-boot.service
echo "[+] xray-first-boot.service disabled (will not run again)"
